...
{ uses algtyp;}
 ...
 function sil( k :integer) :double;
 function Newton(n,k :integer) :double;
...
// function Hex(liczba : Byte) : string;
 ...
 function ilosc_znakow (znak :char; wiersz :string) :integer;
        {robito samo co powyzej - napisane pozniej}
 function pobierz_element(temp_str :string; numer :byte):string;
       {funkcja zwraca kolejny parametr dwuznakowy z
       listy wartosci np V1,V2,V3. numer=2 -> pobierz_element=V2.
       BEZ SPACJI !!!!}
...
// uses wskaznik ;
 ...
 function sil( k :integer) :double;
{ oblicza silnie liczby k. Max. wart "k" wynosi 12}
 var i  :integer;
     il :double;

 begin
   il := 1;
   if (k = 0) or (k = 1) then sil := 1
   else
      begin
        for i := 1 to k do
        il := il * i;
...
begin
  ...
  if wiersz[i] = znak then inc(j);
  ilosc_znakow := j;
end;

function pobierz_element(temp_str :string; numer :byte):string;
var pozycja      :byte;
    lokal_str, wynik    :String;
    i ,j           :byte;
    elementow    :byte;
begin
  ...
  j:= 0; pozycja := 0;  elementow := 0;
  lokal_str := ',' + temp_str;
  for i := 1 to length(lokal_str) do
  begin
    if (lokal_str[i] = ',') then
    begin
     inc(elementow);
     ...
     begin
       wynik := lokal_str[i+1] + lokal_str[i+2];
       pobierz_element := wynik;
       exit;
     end
