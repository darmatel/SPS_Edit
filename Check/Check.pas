...
function TestujPrzyrostCzasu(dzien_poprzedni, dzien_nastepny:integer;
         czas_nastepny, czas_poprzedni :real) :boolean;function TestujCzas(data, czas :longint):boolean; //Zawiera w sobie obie nastÍpne funkcje
...
begin
  ...
  temp_str2 := '   *' + 'Test plikÛw    : ' + '   ' + nazwa_pliku_S + ' ' +
...
function TestKoduLadunku(KodLadunku :string):boolean;
var   local_charges   :string;// Oczywiúcie w rejestrze;
begin
  if kontrola_poprawnosci then
  begin
...
begin
  ...
  begin
    TestPoprawnosciWierszaS := false;
    Raise ECorrectS.Create('Pierwszy znak wiersza SPS-S musi byÊ "S"');
    Exit;
  end;
...
begin
  ...
  begin
    ...
    Raise ECorrectX.Create('D≥ugoúÊ wiersz SPS-X rÛøna od 80 znakÛw');
    Exit;
  end;
  ...
  begin
    TestPoprawnosciWierszaX := false;
    Raise ECorrectX.Create('Pierwszy znak wiersza SPS-X musi byÊ "X"');
    Exit;
  end;
...
begin
  ...
  begin
    TestPoprawnosciWierszaR := false;
    Raise ECorrectR.Create('Pierwszy znak w SPS-R musi byÊ "R"');
    Exit;
  end;
